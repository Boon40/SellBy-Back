...
class ProductServiceDiffblueTest {
    ...
    /**
     * Method under test: {@link ProductService#getProductsByUserId(int)}
     */
    @Test
    void testGetProductsByUserId() {
        when(productRepository.getProductsByUser(Mockito.<User>any())).thenReturn(new ArrayList<>());
        ...
        List<ProductResponse> actualProductsByUserId = productService.getProductsByUserId(1);
        verify(productRepository).getProductsByUser(Mockito.<User>any());
        verify(userService).getUserById(anyInt());
        assertTrue(actualProductsByUserId.isEmpty());
    }
    ...
    void testAddProduct() {
        ...
        when(productMapper.toEntity(Mockito.<ProductRequest>any(), Mockito.<User>any(), Mockito.<State>any(),
                Mockito.<Category>any())).thenReturn(product2);
        ...
        when(stateService.getStateById(anyInt())).thenReturn(state4);

        Category category4 = new Category();
        category4.setCategory(CategoryEnum.Art);
        category4.setId(1);
        when(categoryService.getCategoryById(anyInt())).thenReturn(category4);
        ...
        verify(productMapper).toEntity(Mockito.<ProductRequest>any(), Mockito.<User>any(), Mockito.<State>any(),
                Mockito.<Category>any());
        verify(productMapper).toResponse(Mockito.<Product>any());
        verify(categoryService).getCategoryById(anyInt());
        verify(stateService).getStateById(anyInt());
        verify(userService).getUserById(anyInt());
