...
class UserServiceDiffblueTest {
    ...
    /**
     * Method under test: {@link UserService#getUserResponseById(int)}
     */
    @Test
    void testGetUserResponseById() throws Exception {
        User user = new User();
        user.setCity("Oxford");
        user.setCountry("GB");
        ...
        UserResponse userResponse = new UserResponse(1, "jane.doe@example.org", "42", "Jane", "Doe", "GB", "Oxford",
                LocalDate.of(1970, 1, 1));

        when(userMapper.toResponse(Mockito.<User>any())).thenReturn(userResponse);
        UserResponse actualUserResponseById = userService.getUserResponseById(1);
        verify(userMapper).toResponse(Mockito.<User>any());
        verify(userRepository).findById(Mockito.<Long>any());
        assertSame(userResponse, actualUserResponseById);
    }
